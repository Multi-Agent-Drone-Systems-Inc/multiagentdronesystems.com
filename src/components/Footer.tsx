...
const Footer: React.FC = () => {
  ...
  // Toggle this to enable/disable newsletter functionality
  const NEWSLETTER_AVAILABLE = false;
  ...
  const handleNewsletterSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!NEWSLETTER_AVAILABLE) return;
    
    // Add newsletter submission logic here when available
    console.log('Newsletter subscription submitted');
  };
  
  return (
    <footer id="footer" className="bg-[#1A1A1A] relative z-60 -mt-20 pt-20">
      {/* Texture overlay */}
      <div 
          ...
          <form onSubmit={handleNewsletterSubmit} className="flex w-full max-w-md">
            <input
              ...
              className={`flex-1 bg-gray-800 text-white px-4 py-2 rounded-l focus:outline-none focus:ring-1 focus:ring-[#FFD700] ${
                !NEWSLETTER_AVAILABLE ? 'cursor-not-allowed opacity-60' : ''
              }`}
              disabled={!NEWSLETTER_AVAILABLE}
              required={NEWSLETTER_AVAILABLE}
            />
            <button
              type="submit"
              disabled={!NEWSLETTER_AVAILABLE}
              className={`px-8 py-2 rounded-r font-medium transition-colors duration-300 ${
                NEWSLETTER_AVAILABLE
                  ? 'bg-[#FFD700] text-gray-900 hover:bg-[#FCC201] cursor-pointer'
                  : 'bg-gray-600 text-gray-400 cursor-not-allowed'
              }`}
            >
              {NEWSLETTER_AVAILABLE ? 'Submit' : 'Unavailable'}
            </button>
          </form>
        </div>
